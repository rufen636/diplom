...
# OrderFlow System - Security Architecture Documentation
...
Эти принципы формируют многоуровневую защиту, где каждый компонент системы вносит свой вклад в общую безопасность.
...
---
В ходе разработки серверной части программного средства был использован Swagger (OpenAPI) – один из наиболее популярных инструментов для созда ния интерактивной документации REST API. Для интеграции Swagger UI в проект на PHP применён пакет l5-swagger, который автоматически формирует спецификацию API и предоставляет веб интерфейс для её просмотра и тестирования.
Swagger UI представлен на рисунке 1.1.
<img width="468" height="234" alt="image" src="https://github.com/user-attachments/assets/feb46ef3-3cf1-4fe0-8f51-9c7750ea24a9" />

Рисунок 1 – Swagger UI

JSON с описанием всего API можно получить, перейдя по ссылке http://127.0.0.1:[номер порта]/api/documentation. Этот файл содержит полную спецификацию OpenAPI. 
В совокупности эти инструменты делают процесс разработки более предсказуемым и эффективным, сокращая время интеграции и повышая качество конечного продукта.




2  РЕЗУЛЬТАТЫ ОЦЕНКИ КАЧЕСТВА КОДА РАЗРАБОТАННОГО ПРОГРАММНОГО СРЕДСТВА

Для анализа качества программного обеспечения, реализованного на языке Php с использованием фреймворка Laravel, были рассмотрены современные подходы и публикации, посвящённые метрикам сопровождаемости и архитектурной устойчивости кода. В практике Php разработки применяются следующие показатели:
1 	Цикломатическая сложность (Cyclomatic Complexity) – отражает количество независимых путей выполнения в методе. Чем выше значение, тем сложнее тестирование и сопровождение; оптимальным считается диапазон до 10–15.
2 	Связанность классов (Class Coupling) – показывает число внешних зависимостей. Высокая связанность усложняет модульное тестирование и повышает вероятность ошибок при изменениях.
3 	Объём кода (Lines of Code, LOC) – характеризует количество строк исполняемого кода. Более компактная реализация при сохранении функциональности свидетельствует об эффективности.
4 	Дублирование кода (Code Duplication) – повторяющиеся фрагменты затрудняют сопровождение и увеличивают риск ошибок; их устранение повышает качество архитектуры.
5 	Потенциальные ошибки и уязвимости – статический анализ выявляет неиспользуемые элементы, небезопасные конструкции и нарушения соглашений о стиле.
Для проведения анализа использовались встроенные инструменты среды разработки PhpStorm , включая Inspect Code, Analyze Dependencies и Locate Duplicates. Полученные результаты представлены на рисунках 4–6.


 <img width="468" height="307" alt="image" src="https://github.com/user-attachments/assets/dfcea31b-41d2-4cd2-87f4-82813376883c" />


Рисунок 2 – Статический анализ кода (Inspect Code)

Статический анализ кода (Inspect Code) выявил 77 предупреждений, 800 слабых предупреждения и 15 орфографических ошибки. 
<img width="468" height="173" alt="image" src="https://github.com/user-attachments/assets/c9b45b4b-ab02-437b-8e37-2a8542ae92f5" />

 

Рисунок 3 – Анализ связности и зависимостей

Анализ связности и зависимостей показал, что проект организован по слоям: app, routes, vendor, config, и database. Наиболее насыщенными по количеству классов являются пакеты app.controllers, vendor и app.models, что указывает на активное использование паттерна CQRS. Зависимости между слоями соблюдают принцип разделения ответственности, однако выявлены участки с избыточной связностью, особенно в контроллерах, где нарушается инкапсуляция логики.
<img width="468" height="253" alt="image" src="https://github.com/user-attachments/assets/da9e126a-787e-4646-9692-544904fdca43" />

 

Рисунок 4 – Анализ дублирования

Анализ дублирования кода выявил повторяющиеся фрагменты в нескольких классах, включая TariffsController.php. Основные дубликаты связаны с обработкой валидации. Такое повторение усложняет сопровождение, увеличивает риск ошибок при изменениях и снижает читаемость проекта.


Unit-тестирование применялось для проверки корректности работы отдельных методов бизнес-логики, используемых в системе управления договорами на предоставление услуг доступа к сети «Интернет». Целью данного этапа было подтверждение того, что каждый модуль ведёт себя предсказуемо и возвращает корректные результаты при заданных входных данных. В процессе написания unit-тестов использовались PHPUnit и Laravel Testing, позволяющие изолировать тестируемые методы от внешних зависимостей и проверять их работу независимо от состояния базы данных или других сервисов.
Тестированию подверглись ключевые функции, связанные с управлением договорами и тарифами. Для модуля договоров тестирование включало валидацию данных договоров, проверку уникальности номера договора, корректности дат начала и окончания, проверку корректности диапазона дат, чтобы конечная дата была позже начальной. Также тестировался расчёт финансовых показателей, проверка корректности сумм договоров, управление статусами договоров, включая активный, завершённый и расторгнутый статусы. Тестировалась связь договоров с клиентами, как с физическими, так и с юридическими лицами, а также фильтрация и поиск договоров по номеру, статусу и клиенту.

1 Тест корректности записи договора.
```php
   /** @test */
...
```
Данный тест проверяет корректность записи в базу данных контракта. Метод должен учитывать все валидации.

2 Тест сортировки тарифов.
```php
/** @test */
...
```
Метод тестирует работоспособность сортировки тарифов по параметрам.
3 Тест обновления тарифов.
```php
/** @test */
...
```
Тест возможность обновления тарифов.
...
```
Этот тест проверяет данные и работу аунтификации.

1.2 Интеграционные тесты

Интеграционное тестирование применялось для проверки взаимодействия различных компонентов системы управления договорами: контроллеров, сервисов, моделей и маршрутов, а также корректности обработки HTTP-запросов через RESTful API. Для интеграционных тестов использовались Laravel Testing и PHPUnit с расширенными возможностями для тестирования HTTP-слоя.
Целью интеграционного тестирования было убедиться, что HTTP-эндпоинты корректно принимают входные параметры, вызывают соответствующие методы контроллеров, взаимодействуют с сервисным слоем и возвращают ожидаемый формат ответа в соответствии с требованиями системы. Тестирование охватывало полный цикл обработки запросов - от получения HTTP-запроса до формирования HTTP-ответа, включая промежуточные слои аутентификации и валидации.

1 Тест аунтификации с фальшивым паролем через API.
```php
...
```
Тест отправляет HTTP-запрос к /login и проверяет структуру ответа. Это подтверждает, что контроллер корректно работает, возвращает ошибку.

2 Тест на проверку положительной цены.
```php
/** @test */
...
```
Проверяется, что цена у тарифа положительная: контроллер – сервис – ответ API..
3 Тест на удаление тарифа.
```php
  /** @test */
...
```
Проверяется, что API корректно обрабатывает удаление тарифа.
